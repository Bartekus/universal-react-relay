...
const schemaMarkdownPath = path.resolve('server/data', 'TUTORIAL.md');
...
${schemaMarkdownFile
  ...
  .reduce((sql, line) => {
    // If we are inside some SQL code,
    // we want to include this code in our final file.
    if (isInsideMarkdownSQLCode) {
      // If this line closes our SQL,
      // do not include this line, but do change our state.
      if (line === '```') {
    ...
    // If we are not inside some SQL code,
    // we should just return back the code so far.
...
fs.writeFileSync(path.resolve('server/data', 'schema.sql'), schemaSQLFile, 'utf8');
